describe RewardsBamboohr::Base do
  describe "bonus create" do
    ...
    before do
      allow(RewardsBamboohr::BamboohrFetcher)
        .to receive(:new)
          .with("flatstack", "bamboo-key") { bamboohr_fetcher_double }

      allow(Rewards::Client).to receive(:new) { rewards_client }

      allow(rewards_client)
        .to receive(:bot_create_token) { fixture("rewards_token") }

      allow(RewardsBamboohr::Bonus)
        .to receive(:new)
          .with(
            token: "token-id",
            template: template
